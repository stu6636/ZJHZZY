...
client = OpenAI(
    api_key="sk-GLpfAHbV0fjg6TsqwrnMfY9auSgD1YbZbNm5VSTbinAWyBro",  # 请替换为你的 API Key
...
def main():
    ...
    split = os.getenv("ALFWORLD_SPLIT", "train")
    ...
    progress_path = os.path.join(base_dir, "bloom_dist_progress_train.json")
    ...
    try:
        for idx in range(start_idx, 100):
            ...
            for i, (task_key, task_tag) in enumerate(PREFIXES.items()):
                ...
                init_prompt = f"Interact with a household to solve a task.Here are two examples.\n" + examples + "Here is the task.\n" +ob_text + "\n>"
                #print(init_prompt)
                ...
                if not (done1 and r1 > 0):
                    ...
                    for attempt_idx, reason in enumerate(reason_candidates, start=1):
                        ...
                        ob2_text = "\n".join(ob2[0].split("\n\n")[1:])
                        retry_prompt = f"Interact with a household to solve a task.Here are two examples.\n" + examples + "Here is the task.\n" +ob2_text + "\n>"
    ...
    result_path = os.path.join(base_dir, "alf_bloom_result_100train.txt")
